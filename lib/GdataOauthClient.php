...
// to include Zend library for GData and Oauth
...
class GdataOauthClient extends Zend_Oauth_Consumer {
  ...
  /**
   * Create GdataOauthClient object
   *
   ...
   */
  public function __construct($consumerKey='anonymous', $consumerSecret='anonymous',
                              $sigMethod='HMAC-SHA1') {
  ...
  public function fetchAccessToken() {
    if (!isset($_SESSION['GOOGLEDOCS_ACCESS_TOKEN'])) {
        if (!empty($_GET) && isset($_SESSION['GOOGLEDOCS_REQUEST_TOKEN'])) {
            return parent::getAccessToken(
                $_GET, unserialize($_SESSION['GOOGLEDOCS_REQUEST_TOKEN']));
        }
    }
    return null;
  }
